besogo.makeBoardDisplay = function(container, editor) {
    ...
    function isAnalysisMark(mark) {
        return mark && typeof mark === 'object' && mark.type === 'analysis' && mark.color;
    }
        ...
        for (i = 1; i <= sizeX; i++) {
            for (j = 1; j <= sizeY; j++) {
                ...
                if (mark) {
                    ...
                    } else if (typeof mark === 'number') { // Markup is a basic shape
                        switch(mark) {
                            case 1:
                                element = besogo.svgCircle(x, y, color);
                                break;
